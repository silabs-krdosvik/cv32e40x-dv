...
volatile unsigned int num_misaligned_LRW_trapped;
volatile unsigned int num_misaligned_SCW_trapped;
volatile unsigned int unexpected_irq_beh;
...
int test_LR_SC_sequence_failure_address_mismatch(void)
{
  ...
  volatile uint32_t mem_addr_LRW;
  volatile uint32_t mem_addr_SCW;
  volatile uint32_t is_failure;
  volatile uint32_t random;
...
int test_LRW_misaligned_access(void)
{
  ...
  __asm__ volatile("lr.w %0, (%1)" : "=r" (random) : "r" (mem_addr_misaligned));
...
int test_SCW_misaligned_access(void)
{
  ...
  __asm__ volatile("sc.w %[is_failure], %1, (%2)" : [is_failure]"=r" (is_failure) : "r" (random), "r" (mem_addr_misaligned));


  //(Execute exception handler)
  //printf("Expected SCW to trap due to misaligned address, nr=. However, it did not.\n");
  if (num_misaligned_SCW_trapped != 1) {
    printf("Expected SCW to trap due to misaligned address, nr=%i. However, it did not.\n", num_misaligned_SCW_trapped);
    return 1;
  }
